...
def _process_common_values(df_tel, col, hpso_dict):
    ...
    """
    stations = int(df_tel.loc[['Stations/antennas'], col].iloc[0])
    hpso_dict['Stations'] = stations
    baseline = float(df_tel.loc[['Max Baseline [km]'], col].iloc[0]) * 1000
    hpso_dict['Baseline'] = baseline
    channels = float(df_tel.loc[['Max # of channels'], col].iloc[0])
    hpso_dict['Channels'] = channels
    t_obs = float(df_tel.loc[['Observation Time [s]'], col].iloc[0])
    hpso_dict['Tobs [h]'] = t_obs / 3600
    t_exp = float(df_tel.loc[['Total Time [s]'], col].iloc[0])
...
def _isolate_products(df_tel, hpso):
    ...
    for pl in [col for col in df_tel.columns if hpso in col]:
        ...
        for product in PRODUCTS:
            ...
            if compute_entry.iloc[0] == ' ' or compute_entry.iloc[0] == '':
                pipeline_overview[product] = -1
            else:
                pipeline_overview[product] = float(compute_entry.iloc[0])

            if data_entry.iloc[0] == ' ' or data_entry.iloc[0] == '':
                pipeline_data_overview[product] = -1
            else:
                # Each visibility
                pipeline_data_overview[product] = float(data_entry.iloc[0])
...
if __name__ == '__main__':
    ...
    IN_DIR = Path('skaworkflows/data/sdp-par-model_output/')
    ...
    total_sizing, component_sizing = compile_sizing(IN_DIR.iterdir())
